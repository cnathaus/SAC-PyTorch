...
class SAC_Agent:
    ...
    def __init__(self, seed, state_dim, action_dim, lr=3e-4, gamma=0.99, tau=5e-3, batchsize=256, hidden_size=256, update_interval=1):
        ...
        self.q_funcs = DoubleQFunc(
            state_dim, action_dim, hidden_size=hidden_size).to(device)
    ...
    def update_policy_and_temp(self, state_batch):
        action_batch, logprobs_batch, _ = self.policy(
            state_batch, get_logprob=True)
        ...
        for i in range(n_updates):
            ...
            if state_filter:
                state_batch = torch.FloatTensor(
                    state_filter(samples.state)).to(device)
            ...
            done_batch = torch.FloatTensor(
                samples.done).to(device).unsqueeze(1)
            ...
            for p in self.q_funcs.parameters():
                p.requires_grad = False
            pi_loss_step, a_loss_step = self.update_policy_and_temp(
                state_batch)
